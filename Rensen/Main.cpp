...
#include "C:\Users\devil\Desktop\KeyAuth-CPP-Example-main\auth.hpp"
#include "C:\Users\devil\Desktop\KeyAuth-CPP-Example-main\utils.hpp"
#include "C:\Users\devil\Desktop\KeyAuth-CPP-Example-main\json.hpp"
#include "C:\Users\devil\Desktop\KeyAuth-CPP-Example-main\skStr.h"
...
auto name = skCrypt("devire"); // application name. right above the blurred text aka the secret on the licenses tab among other tabs
auto ownerid = skCrypt("mcAPRxMyEa"); // ownerid, found in account settings. click your profile picture on top right of dashboard and then account settings.
auto secret = skCrypt("d714b0cbb5c08a6dea1109c63559d97e35f20fd3c222239b73cb67f1ae3dc220"); // app secret, the blurred text on licenses tab and other tabs
auto version = skCrypt("2.0"); // leave alone unless you've changed version on website
...
namespace Control_Var//套用到菜单的调试变量 (例如功能开关)
{
	...
	void LoadPreset(string FileName = "") noexcept//加载特定预设
	{
		...
		UI_Misc_ThirdPerson = Variable::string_int_(System::Get_File(FileName, 144));
	}
...
void Thread_Menu() noexcept//菜单线程 (提供给使用者丰富的自定义选项)
{
	...
	while (true)//默认英文版菜单
	{
		...
		if (!GUI_VAR.Window_Move() && Menu_Open)//不在移动窗口时绘制GUI
		{
			...
			{
				const auto Block_Misc = GUI_VAR.GUI_Block(150, 30, 805, "Miscellaneous");
				GUI_VAR.GUI_Checkbox(Block_Misc, 1, "Bunny hop", UI_Misc_BunnyHop);
				GUI_VAR.GUI_Checkbox(Block_Misc, 26, "ThirdPerson", UI_Misc_ThirdPerson);
...
void Thread_Funtion_BunnyHop() noexcept//功能线程: 连跳
{
	...
	while (true)
	{
		if (CS2_HWND && Global_IsShowWindow && Global_LocalPlayer.Health() && UI_Misc_BunnyHop && System::Get_Key(VK_SPACE))
		{
			...
			Sleep(System::Rand_Number(1, 12, System::Tick()));
		}
		else Sleep(20);
	}
}
void Thread_Function_ThirdPersonCamera() noexcept//第三人称视角
{
	System::Log("Load Thread: Thread_ThirdPersonCamera()");
	while (true)
	{
		Variable::Vector3 cameraPosition;
		Variable::Vector3 cameraTarget;
		float cameraDistance = 5.0f; // Abstand zur Kamera
		float cameraHeight = 2.0f;   // Höhe der Kamera


		if (CS2_HWND && Global_IsShowWindow && Global_LocalPlayer.Health() && UI_Misc_ThirdPerson)
		{
			static auto Old_ThirdPersonCamera = false;
			if (System::Get_Key(UI_Misc_ThirdPerson))Old_ThirdPersonCamera = !Old_ThirdPersonCamera;
			if (Old_ThirdPersonCamera)ExecuteCommand("thirdperson");
			else ExecuteCommand("firstperson");
		}
		Sleep(20);
	}
}
...
int main() noexcept//主线程 (加载多线程, 一些杂项功能)
{
	...
	std::string consoleTitle = skCrypt("devire - Built at:  ").decrypt() + compilation_date + " " + compilation_time;
	...
	if (std::filesystem::exists("key.json")) //change key.txt to the path of your file :smile:
	{
	...
	else
	{
		...
		{
			WriteToJson("key.json", "username", username, true, "password", password);
			std::cout << skCrypt("Successfully Created File For Auto Login");
		}
	...
	thread Thread_ThirdPersonCamera = thread(Thread_Function_ThirdPersonCamera);
